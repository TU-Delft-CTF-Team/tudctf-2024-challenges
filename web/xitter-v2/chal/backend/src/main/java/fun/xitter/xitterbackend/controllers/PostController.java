...
public class PostController {
    ...
    @GetMapping("/user/{id}")
    public ResponseEntity<List<PostDTO>> getPostsByUser(@PathVariable Long id, Authentication authentication) {
        return ResponseEntity.ok(postRepository.findByAuthorId(id,
                userManager.getUserFromAuthentication(authentication)).stream().map(PostDTO::new).toList());
    }
}
