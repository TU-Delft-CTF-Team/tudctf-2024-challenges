...
async def post_index(
    ...
    product_price: Annotated[str, Form()],  # Get the product price from the form
):
    global USER_DATA

    actual_product_price = float(product_price)

    # Convert the number to a float
    if actual_product_price < 0:
        return get_response(request, "You can't buy a product for a negative price")

    # Check if the product id is a donation
    if product_id == 999:
        USER_DATA.balance -= actual_product_price
        USER_DATA.products.append(
            Product(
                id=999,
                name=f"Donation (${actual_product_price})",
        ...
        return get_response(request, "Thank you for your donation!")
